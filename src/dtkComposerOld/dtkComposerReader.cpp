 ...
 * Version: $Id$
 * Last-Updated: Tue Dec  6 13:56:21 2011 (+0100)
 *           By: Julien Wintz
 *     Update #: 1030
 */
...
bool dtkComposerReader::read(const QString& fileName, bool append)
{
    ...
    for(int i = 0; i < edges.count(); i++) {
        ...
        // -- dealing with composite nodes

        if(d->node_map.value(source_id)->kind() == dtkComposerNode::Composite)
            source_property_id = source_id;

        if(d->node_map.value(destin_id)->kind() == dtkComposerNode::Composite)
            destin_property_id = destin_id;

        // -- dealing with control nodes
        ...
        if (source_property_id.isEmpty() && destin_property_id.isEmpty() && source_property_block.isEmpty() && destin_property_block.isEmpty()) {
            ...
            if (l->parentNode() == r) {
                ...
                edge->setSource(dynamic_cast<dtkComposerNodeControl *>(d->node_map.value(source_id))->rightProperty(source_property_block, source_property));
                edge->setDestination(dynamic_cast<dtkComposerNodeControl *>(d->node_map.value(destin_id))->rightProperty(destin_property_block, destin_property));

            } else if (r->parentNode() == l) {

                edge->setSource(dynamic_cast<dtkComposerNodeControl *>(d->node_map.value(source_id))->leftProperty(source_property_block, source_property));
                edge->setDestination(dynamic_cast<dtkComposerNodeControl *>(d->node_map.value(destin_id))->leftProperty(destin_property_block, destin_property));               
            ...
            if (!destin_property_id.isEmpty()) {
                ...
                if (l->parentNode() == r) {
                    ...
                    edge->setSource(dynamic_cast<dtkComposerNodeControl *>(d->node_map.value(source_id))->rightProperty(source_property_block, source_property));
                    // edge->setDestination(d->node_map.value(destin_id)->rightProperty(destin_property, d->node_map.value(destin_property_id.toInt())));
                    edge->setDestination(d->node_map.value(destin_id)->g->rightProperty(destin_property));
                    ...
                    // edge->setDestination(d->node_map.value(destin_id)->leftProperty(destin_property, d->node_map.value(destin_property_id.toInt())));
                    edge->setDestination(d->node_map.value(destin_id)->g->leftProperty(destin_property));
                }
            ...
            if (!source_property_id.isEmpty()) {
                ...
                if (r->parentNode() == l) {
                    ...
                    // edge->setSource(d->node_map.value(source_id)->leftProperty(source_property, d->node_map.value(source_property_id.toInt())));
                    edge->setSource(d->node_map.value(source_id)->g->leftProperty(source_property));
                    edge->setDestination(dynamic_cast<dtkComposerNodeControl *>(d->node_map.value(destin_id))->leftProperty(destin_property_block, destin_property)); 

                } else {

                    // edge->setSource(d->node_map.value(source_id)->rightProperty(source_property, d->node_map.value(source_property_id.toInt())));
                    edge->setSource(d->node_map.value(source_id)->g->rightProperty(source_property));
...
dtkComposerNode *dtkComposerReader::readNode(QDomNode node)
{
    ...
    if(dtkComposerNodeControl *control_node = dynamic_cast<dtkComposerNodeControl *>(n)) {
        ...
        // -- According to the type of control node, some properties
        //    are treated using special cares. Hence, they are
        //    registered in a protected list.

        QStringList protected_properties;

        if (dtkComposerNodeCase *case_node = dynamic_cast<dtkComposerNodeCase *>(control_node)) {

            // -- In the case of Case control node, we need to add the
            //    blocks after parsing the tree.
            ...
            for(int i = 0; i < case_block_count; i++)
                case_node->createBlock(QString("case%1").arg(i));

            protected_properties << "variable" << "constant";

        } else if (dynamic_cast<dtkComposerNodeConditional *>(control_node)) {
            protected_properties << "condition";

        } else if (dynamic_cast<dtkComposerNodeLoopDataComposite *>(control_node)) {
            protected_properties << "from" << "to" << "step" << "item" << "index";

        } else if (dynamic_cast<dtkComposerNodeLoopFor *>(control_node)) {
            protected_properties << "variable" << "condition";

        } else if (dynamic_cast<dtkComposerNodeLoopWhile *>(control_node)) {
            protected_properties << "condition";

        }

        // -- Now, properties can be added in the same way for all
        //    types of control node.
        ...
        for(int i = 0; i < children.count(); i++) {
            ...
            if (protected_properties.contains(children.at(i).toElement().attribute("name")))
                continue;

            dtkComposerNodeControlBlock *current_block = control_node->block(children.at(i).toElement().attribute("block"));

            if (children.at(i).toElement().attribute("behavior") == "asinput")
                behavior = dtkComposerNodeProperty::AsInput;
            else if (children.at(i).toElement().attribute("behavior") == "asoutput")
                behavior = dtkComposerNodeProperty::AsOutput;
            else if (children.at(i).toElement().attribute("behavior") == "asrelay")
                behavior = dtkComposerNodeProperty::AsRelay;
            else if (children.at(i).toElement().attribute("behavior") == "asloop")
                behavior = dtkComposerNodeProperty::AsLoop;

            if (children.at(i).toElement().attribute("multiplicity") == "null")
                multiplicity = dtkComposerNodeProperty::Null;
            else if (children.at(i).toElement().attribute("behavior") == "single")
                multiplicity = dtkComposerNodeProperty::Single;
            else if (children.at(i).toElement().attribute("behavior") == "multiple")
                multiplicity = dtkComposerNodeProperty::Multiple;

            if(children.at(i).toElement().attribute("position") == "left") {

                control_node->appendBlockLeftProperty(children.at(i).toElement().attribute("name"), behavior, multiplicity, current_block);

            } else if(children.at(i).toElement().attribute("position") == "right") {

                control_node->appendBlockRightProperty(children.at(i).toElement().attribute("name"), behavior, multiplicity, current_block);
    ...
    { // -- properties
        ...
        for(int i = 0; i < children.count(); i++) {
            ...
            QString position = children.at(i).toElement().attribute("position");
            QString behavior = children.at(i).toElement().attribute("behavior");
            QString multiplicity = children.at(i).toElement().attribute("multiplicity");
            ...
            if(type == "generic" && n->kind() == dtkComposerNode::Composite) {

                dtkComposerNodeProperty::Position p_position;
                if (position == "right")
                    p_position = dtkComposerNodeProperty::Right;
                else
                    p_position = dtkComposerNodeProperty::Left;

                dtkComposerNodeProperty::Behavior p_behavior;
                if(behavior == "asrelay")
                    p_behavior = dtkComposerNodeProperty::AsRelay;

                dtkComposerNodeProperty::Multiplicity p_multiplicity;
                if(multiplicity == "multiple")
                    p_multiplicity = dtkComposerNodeProperty::Multiple;
                else
                    p_multiplicity = dtkComposerNodeProperty::Single;

                dtkComposerNodeProperty *property = new dtkComposerNodeProperty(name, p_position, p_behavior, p_multiplicity, n);

                if(position == "left")
                    n->g->appendLeftProperty(property);

                if(position == "right")
                    n->g->appendRightProperty(property);
