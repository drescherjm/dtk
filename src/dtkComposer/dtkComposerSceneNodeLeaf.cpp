 ...
 * Version: $Id$
 * Last-Updated: Mon Feb 27 15:40:12 2012 (+0100)
 *           By: tkloczko
 *     Update #: 190
 */
...
void dtkComposerSceneNodeLeaf::wrap(dtkComposerNode *node)
{
    ...
    foreach(dtkComposerTransmitter *receiver, node->receivers()) {
        ...
        port->setLabel(node->labelHint(this->inputPorts().indexOf(port)));
    ...
    foreach(dtkComposerTransmitter *emitter, node->emitters()) {
        ...
        port->setLabel(node->labelHint(this->outputPorts().indexOf(port)));
