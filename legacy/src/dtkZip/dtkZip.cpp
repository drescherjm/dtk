 ...
 * Created: Fri Sep 23 12:39:41 2011 (+0200)
 * Version: $Id$
 * Last-Updated: Fri Sep 23 13:11:56 2011 (+0200)
 *           By: Julien Wintz
 *     Update #: 16
 */
...
#include "dtkZipReader.h"
#include "dtkZipWriter.h"
...
dtkZipReader::FileInfo::FileInfo()
...
dtkZipReader::FileInfo::FileInfo(const FileInfo &other)
{
    operator=(other);
}

dtkZipReader::FileInfo& dtkZipReader::FileInfo::operator=(const FileInfo &other)
{
...
bool dtkZipReader::FileInfo::isValid() const
{
...
{
public:
    dtkZipPrivate(QIODevice *device, bool ownDev)
    ...
    void fillFileInfo(int index, dtkZipReader::FileInfo &fileInfo) const;
...
void dtkZipPrivate::fillFileInfo(int index, dtkZipReader::FileInfo &fileInfo) const
{
...
class dtkZipReaderPrivate : public dtkZipPrivate
{
public:
    dtkZipReaderPrivate(QIODevice *device, bool ownDev)
        : dtkZipPrivate(device, ownDev), status(dtkZipReader::NoError)
    {
    }

    void scanFiles();

    dtkZipReader::Status status;
};

class dtkZipWriterPrivate : public dtkZipPrivate
{
public:
    dtkZipWriterPrivate(QIODevice *device, bool ownDev)
        : dtkZipPrivate(device, ownDev),
        status(dtkZipWriter::NoError),
        permissions(QFile::ReadOwner | QFile::WriteOwner),
        compressionPolicy(dtkZipWriter::AlwaysCompress)
    {
    }

    dtkZipWriter::Status status;
    QFile::Permissions permissions;
    dtkZipWriter::CompressionPolicy compressionPolicy;
...
void dtkZipReaderPrivate::scanFiles()
{
    if (!dirtyFileTree)
        return;

    if (! (device->isOpen() || device->open(QIODevice::ReadOnly))) {
        status = dtkZipReader::FileOpenError;
        return;
    }

    if ((device->openMode() & QIODevice::ReadOnly) == 0) { // only read the index from readable files.
        status = dtkZipReader::FileReadError;
        return;
    }
    ...
    if (readUInt(tmp) != 0x04034b50) {
        qWarning() << "dtkZip: not a zip file!";
        return;
    }
    ...
    while (start_of_directory == -1) {
        ...
        if (pos < 0 || i > 65535) {
            qWarning() << "dtkZip: EndOfDirectory not found";
            return;
        }
    ...
    if (comment_length != i)
        qWarning() << "dtkZip: failed to parse zip file.";
    ...
    for (i = 0; i < num_dir_entries; ++i) {
        ...
        if (read < (int)sizeof(CentralFileHeader)) {
            qWarning() << "dtkZip: Failed to read complete header, index may be incomplete";
            break;
        }
        if (readUInt(header.h.signature) != 0x02014b50) {
            qWarning() << "dtkZip: invalid header signature, index may be incomplete";
            break;
        }
        ...
        if (header.file_name.length() != l) {
            qWarning() << "dtkZip: Failed to read filename from zip index, index may be incomplete";
            break;
        }
        l = readUShort(header.h.extra_field_length);
        header.extra_field = device->read(l);
        if (header.extra_field.length() != l) {
            qWarning() << "dtkZip: Failed to read extra field in zip file, skipping file, index may be incomplete";
            break;
        }
        l = readUShort(header.h.file_comment_length);
        header.file_comment = device->read(l);
        if (header.file_comment.length() != l) {
            qWarning() << "dtkZip: Failed to read read file comment, index may be incomplete";
            break;
        }
...
void dtkZipWriterPrivate::addEntry(EntryType type, const QString &fileName, const QByteArray &contents/*, QFile::Permissions permissions, dtkZip::Method m*/)
{
#ifndef NDEBUG
    ...
    if (! (device->isOpen() || device->open(QIODevice::WriteOnly))) {
        status = dtkZipWriter::FileOpenError;
        return;
    }
    device->seek(start_of_directory);

    // don't compress small files
    dtkZipWriter::CompressionPolicy compression = compressionPolicy;
    if (compressionPolicy == dtkZipWriter::AutoCompress) {
        if (contents.length() < 64)
            compression = dtkZipWriter::NeverCompress;
        else
            compression = dtkZipWriter::AlwaysCompress;
    }
    ...
    if (compression == dtkZipWriter::AlwaysCompress) {
       ...
       ulong len = contents.length();
        ...
        do {
            ...
            case Z_OK:
                ...
                break;
            case Z_MEM_ERROR:
                qWarning("dtkZip: Z_MEM_ERROR: Not enough memory to compress file, skipping");
                data.resize(0);
                break;
            case Z_BUF_ERROR:
                len *= 2;
...
// TODO add a check if data.length() > contents.length().  Then try to store the original and revert the compression method to be uncompressed
    ...
    if (header.file_name.size() > 0xffff) {
        qWarning("dtkZip: Filename too long, chopping it to 65535 characters");
...
/*!
    ...
    \brief the dtkZipReader class provides a way to inspect the contents of a zip
    archive and extract individual files from it.

    dtkZipReader can be used to read a zip archive either from a file or from any
...
*/
dtkZipReader::dtkZipReader(const QString &archive, QIODevice::OpenMode mode)
{
    ...
    d = new dtkZipReaderPrivate(f.data(), /*ownDevice=*/true);
    f.take();
    d->status = status;
}
 ...
 */
dtkZipReader::dtkZipReader(QIODevice *device)
    : d(new dtkZipReaderPrivate(device, /*ownDevice=*/false))
{
    Q_ASSERT(device);
}
...
*/
QIODevice* dtkZipReader::device() const
{
    return d->device;
}
...
*/
bool dtkZipReader::isReadable() const
{
...
*/
bool dtkZipReader::exists() const
{
...
*/
QList<dtkZipReader::FileInfo> dtkZipReader::fileInfoList() const
{
    d->scanFiles();
...
/*!
    ...
    \sa fileInfoList()
*/
dtkZipReader::FileInfo dtkZipReader::entryInfoAt(int index) const
{
    d->scanFiles();
    dtkZipReader::FileInfo fi;
...
*/
QByteArray dtkZipReader::fileData(const QString &fileName) const
{
    d->scanFiles();
    int i;
    ...
    if (compression_method == 0) {
        ...
        do {
            ...
            case Z_OK:
                ...
                break;
            case Z_MEM_ERROR:
                qWarning("dtkZip: Z_MEM_ERROR: Not enough memory");
                break;
            case Z_BUF_ERROR:
                len *= 2;
                break;
            case Z_DATA_ERROR:
                qWarning("dtkZip: Z_DATA_ERROR: Input data is corrupted");
                break;
            }
        } while (res == Z_BUF_ERROR);
        return baunzip;
    }
    qWarning() << "dtkZip: Unknown compression method";
    return QByteArray();
}
...
*/
bool dtkZipReader::extractAll(const QString &destinationDir) const
{
...
/*!
    Returns a status code indicating the first error that was met by dtkZipReader,
    or dtkZipReader::NoError if no error occurred.
*/
dtkZipReader::Status dtkZipReader::status() const
{
    return d->status;
}
...
/*!
    ...
    \brief the dtkZipWriter class provides a way to create a new zip archive.

    dtkZipWriter can be used to create a zip archive containing any number of files
...
/*!
    ...
    \sa isValid()
*/
dtkZipWriter::dtkZipWriter(const QString &fileName, QIODevice::OpenMode mode)
{
    ...
    if (f->error() == QFile::NoError)
        ...
        if (f->error() == QFile::WriteError)
            status = dtkZipWriter::FileWriteError;
        else if (f->error() == QFile::OpenError)
            status = dtkZipWriter::FileOpenError;
        else if (f->error() == QFile::PermissionsError)
            status = dtkZipWriter::FilePermissionsError;
        else
            status = dtkZipWriter::FileError;
    }

    d = new dtkZipWriterPrivate(f.data(), /*ownDevice=*/true);
    f.take();
    d->status = status;
}
 ...
 */
dtkZipWriter::dtkZipWriter(QIODevice *device)
    : d(new dtkZipWriterPrivate(device, /*ownDevice=*/false))
{
    Q_ASSERT(device);
}

dtkZipWriter::~dtkZipWriter()
{
    close();
    delete d;
}
...
*/
QIODevice* dtkZipWriter::device() const
{
    return d->device;
}
...
*/
bool dtkZipWriter::isWritable() const
{
...
/*!
    Returns true if the file exists; otherwise returns false.
*/
bool dtkZipWriter::exists() const
{
    QFile *f = qobject_cast<QFile*> (d->device);
    if (f == 0)
        return true;
...
/*!
    Returns a status code indicating the first error that was met by dtkZipWriter,
    or dtkZipWriter::NoError if no error occurred.
*/
dtkZipWriter::Status dtkZipWriter::status() const
{
    return d->status;
}

/*!
    \enum dtkZipWriter::CompressionPolicy
...
/*!
    ...
    \sa addFile()
*/
void dtkZipWriter::setCompressionPolicy(CompressionPolicy policy)
{
...
/*!
    ...
    \sa addFile()
*/
dtkZipWriter::CompressionPolicy dtkZipWriter::compressionPolicy() const
{
...
/*!
    ...
    \sa addFile()
*/
void dtkZipWriter::setCreationPermissions(QFile::Permissions permissions)
{
...
/*!
    ...
    \sa addFile()
*/
QFile::Permissions dtkZipWriter::creationPermissions() const
{
...
/*!
    ...
    \sa setCompressionPolicy()
*/
void dtkZipWriter::addFile(const QString &fileName, const QByteArray &data)
{
    d->addEntry(dtkZipWriterPrivate::File, fileName, data);
}
...
/*!
    ...
    includes the full path in the archive.
*/
void dtkZipWriter::addFile(const QString &fileName, QIODevice *device)
{
    Q_ASSERT(device);
    ...
    d->addEntry(dtkZipWriterPrivate::File, fileName, device->readAll());
    if (opened)
        device->close();
}
...
/*!
    ...
    the \a permissions;
*/
void dtkZipWriter::addDirectory(const QString &dirName)
{
    QString name = dirName;
    // separator is mandatory
    if (!name.endsWith(QDir::separator()))
        name.append(QDir::separator());
    d->addEntry(dtkZipWriterPrivate::Directory, name, QByteArray());
}
...
*/
void dtkZipWriter::addSymLink(const QString &fileName, const QString &destination)
{
    d->addEntry(dtkZipWriterPrivate::Symlink, fileName, QFile::encodeName(destination));
}

/*!
   Closes the zip file.
*/
void dtkZipWriter::close()
{
    ...
    //qDebug("dtkZip::close writing directory, %d entries", d->fileHeaders.size());
