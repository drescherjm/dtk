...
public:
    ...
    dtkIOCompressorPrivate(dtkIOCompressor *q_ptr, QIODevice *device, int compressionLevel, int bufferSize);
    ...
    ZlibByte *buffer;
    State state;
    dtkIOCompressor::StreamFormat streamFormat;
};
...
/*!
    \internal
*/
dtkIOCompressorPrivate::~dtkIOCompressorPrivate()
{
    delete[] buffer;
}
...
*/
void dtkIOCompressorPrivate::flushZlib(int flushMode)
{
    // No input.
...
*/
bool dtkIOCompressorPrivate::writeBytes(ZlibByte *buffer, ZlibSize outputSize)
{
    Q_Q(dtkIOCompressor);
...
/*! \class dtkIOCompressor
    \brief The dtkIOCompressor class is a QIODevice that compresses data streams.

    A dtkIOCompressor object is constructed with a pointer to an
    underlying QIODevice.  Data written to the dtkIOCompressor object
    will be compressed before it is written to the underlying
    QIODevice. Similary, if you read from the dtkIOCompressor object,
    the data will be read from the underlying device and then
    decompressed.

    dtkIOCompressor is a sequential device, which means that it does
    not support seeks or random access. Internally, dtkIOCompressor
...
/*!
    ...
    setting this format, by itself, does not let dtkIOCompressor read
...
/*!
    Constructs a dtkIOCompressor using the given \a device as the underlying device.
...
/*!
    Destroys the dtkIOCompressor, closing it if neccesary.
*/
dtkIOCompressor::~dtkIOCompressor()
{
    Q_D(dtkIOCompressor);
    close();
    delete d;
...
/*!
    ...
    \sa dtkIOCompressor::StreamFormat
*/
void dtkIOCompressor::setStreamFormat(StreamFormat format)
{
    Q_D(dtkIOCompressor);
    ...
    if (format == GzipFormat && checkGzipSupport(ZLIB_VERSION) == false)
        qWarning("dtkIOCompressor::setStreamFormat: zlib 1.2.x or higher is "
...
/*!
    ...
    \sa dtkIOCompressor::StreamFormat
*/
dtkIOCompressor::StreamFormat dtkIOCompressor::streamFormat() const
{
...
*/
bool dtkIOCompressor::isGzipSupported()
{
...
/*!
    \reimp
*/
bool dtkIOCompressor::isSequential() const
{
    return true;
}

/*!
    Opens the dtkIOCompressor in \a mode. Only ReadOnly and WriteOnly is supported.
    ...
    if (read) {
        ...
        if (d->streamFormat == dtkIOCompressor::ZlibFormat) {
            ...
            if (checkGzipSupport(zlibVersion()) == false) {
                setErrorString(QT_TRANSLATE_NOOP("dtkIOCompressor::open", "The gzip format not supported in this version of zlib."));
                return false;
            }
...
/*!
     Closes the dtkIOCompressor, and also the underlying device if it was opened by dtkIOCompressor.
    \sa open()
*/
void dtkIOCompressor::close()
{
    ...
    if (openMode() & ReadOnly) {
        ...
        if (d->state == dtkIOCompressorPrivate::BytesWritten) { // Only flush if we have written anything.
            d->state = dtkIOCompressorPrivate::NoBytesWritten;
...
/*!
    ...
    Each time you call flush, all data written to the dtkIOCompressor is compressed and written to the
    underlying device. Calling this function can reduce the compression ratio. The underlying device
    is not flushed.

    Calling this function when dtkIOCompressor is in ReadOnly mode has no effect.
*/
...
*/
qint64 dtkIOCompressor::readData(char *data, qint64 maxSize)
{
    Q_D(dtkIOCompressor);

    if (d->state == dtkIOCompressorPrivate::EndOfStream)
        return 0;

    if (d->state == dtkIOCompressorPrivate::Error)
        return -1;
    ...
    do {
        ...
        if (d->zlibStream.avail_in == 0) {
            ...
            if (bytesAvalible == -1) {
                ...
                setErrorString(QT_TRANSLATE_NOOP("dtkIOCompressor", "Error reading data from underlying device: ") + d->device->errorString());
                return -1;
            }
    ...
    if (status == Z_STREAM_END) {
        d->state = dtkIOCompressorPrivate::EndOfStream;
...
*/
qint64 dtkIOCompressor::writeData(const char *data, qint64 maxSize)
{
    if (maxSize < 1)
        return 0;
