...
#import "dtkDeviceWiiRemotePrivate.h"
...
//	NSLog(@"IR Data %00x,%00x  %00x,%00x", irData[0].x, irData[0].y, irData[1].x, irData[1].y);
    ...
    if (irData[0].s < 0x0F && irData[1].s < 0x0F) {
        ...
        float angle = atan2(dy, dx); (void) angle;
...
- (BOOL) isLED1Illuminated
{
    return isLED1Illuminated;
}

- (BOOL) isLED2Illuminated
{
    return isLED2Illuminated;
}

- (BOOL) isLED3Illuminated
{
    return isLED3Illuminated;
}

- (BOOL) isLED4Illuminated
{
    return isLED4Illuminated;
}
